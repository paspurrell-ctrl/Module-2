...
\newpage
\begin{abstract}
    This report analyzes three population growth models: exponential, linear, and logistic growth. It explains in depth each differential equation, explicit equation, and corresponding graph. The advantages and limitations are also discussed for each model, specifically focusing on if they are more applicable for short- or long-term biological modeling.
\end{abstract}
...
A linear growth model ensures what exponential cannot; it offers predictability and stability (Statsig). Unlike exponential growth, where the rate of increase depends on the population size, linear growth assumes that the population grows by the same amount in each unit of time (LibreTexts).
\begin{equation}
...
N(t) = N_0 + ct,
\end{equation}
In this equation the N$_0$ represents the initial population size and it can be used find the population size at any time, t. 
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{sage Exponential.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{sage linear.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Logistic Sage.png}
